...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		if( isset( $_FILES ) && @$_SERVER['REQUEST_METHOD'] != 'DELETE'){ 
			foreach($_FILES as $key => $value){
				$param_name = $key;
			}
			$param_name = ($param_name != '') ? $param_name : 'files';
		} else {
			$param_name = 'files';
		}
        ...
        $this->options = array(
            ...
            'upload_url' => $wp_upload_dir_url.'/leads/attachments/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->name).'&action=inbound_attachment_file_upload';
